...
class _AddressDetailScreenState extends State<AddressDetailScreen> {
  ...
  void initState() {
    ...
    _district    = _matchOrFirst(widget.district, _districts);
    _neighborhoods = TurkeyGeoData.getNeighborhoods(_district);
    _neighborhood  = _matchOrFirst(widget.neighborhood, _neighborhoods);
  }
  ...
  String _matchOrFirst(String raw, List<String> list) {
    if (list.isEmpty) return '';
    if (raw.isEmpty) return list.first;
    final lo = raw.toLowerCase();
    for (final o in list) {
      if (o.toLowerCase().contains(lo) || lo.contains(o.toLowerCase())) return o;
    }
    return list.first;
  }
    ...
    return Scaffold(
      ...
      body: Form(
        ...
        child: ListView(padding: const EdgeInsets.all(16), children: [
          ...
          _label('İlçe *'),
          _dropdown(
            value: _districts.contains(_district) ? _district : (_districts.isNotEmpty ? _districts.first : null),
            items: _districts, onChanged: _onDistrictChanged),
          const SizedBox(height: 14),
